...
INNER JOIN
users AS usr ON std.first_name = usr.first_name AND std.last_name = usr.last_name;
...
-- FULL JOIN(両方から取り出して、取得できないものは NULL)
SELECT * FROM students AS std
LEFT JOIN  enrollments AS enr
ON std.id = enr.student_id
...
-- customers,orders,items,stores を紐づける (INNER JOIN)
-- customers.id で並び替える (ORDER BY)
-- customers.id が 10 で、orders.order_date が 2020-08-01 より後に絞り込む (WHERE)
...
-- GROUP BY の紐づけ
SELECT customer_id,SUM(order_amount * order_price) FROM orders AS summary_price
